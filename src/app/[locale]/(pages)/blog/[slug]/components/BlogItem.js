...
function BlogItem({ href, item, image, imageBaseUrl }) {
  return (
    <Link href={href} className="relative w-full mb-2 group cursor-pointer">
      ...
      <h1 className="absolute top-0 left-0 w-full px-5 h-full flex items-center justify-center text-white text-center text-xl bg-black bg-opacity-30 shadow-lg p-4 rounded-md">
        {item.baslik}
      </h1>
    </Link>
  );
