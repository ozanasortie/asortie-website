...
export async function generateMetadata({ params }) {
  ...
  const id = subslug.split("-").pop();
  const data = await fetchProductDetail({ slug: id, lang: locale });
...
const transformWhatsappData = (data) => {
  const languages = {
    ...
    arp: { name: "العربية", flag: arab },
    ru: { name: "Pусский", flag: russia },
...
const WhatsappCard = ({ item }) => (
  ...
  >
    <div className="flex items-center mb-3 border-b pb-3 pt-1">
      <img className="mr-2" width={22} src={item.flag} alt={item.language} />
...
const WhatsappNumbers = ({ t, whatsappNumbers }) => {
  return (
    <div className="max-w-[1500px] 2xl:max-w-[1700px] w-[90%] p-4 flex-col mt-10">
...
export default async function ProductDetail({ params }) {
  ...
  return (
    <div className="w-full relative flex flex-col items-center pb-16 overflow-x-hidden pt-24">
      ...
      />
      <Transition className="max-w-[1500px] 2xl:max-w-[1700px] w-[97%] lg:w-[90%] bg-cover bg-center bg-no-repeat flex flex-col lg:flex-row llg:justify-center text-black p-4">
      ...
      <div
        className="max-w-[1500px] 2xl:max-w-[1700px] hidden lg:block text-start w-[97%] lg:w-[90%] px-6 text-gray-800 mt-24"
        dangerouslySetInnerHTML={{ __html: data.Urunler[0].urun_detayi }}
      />
