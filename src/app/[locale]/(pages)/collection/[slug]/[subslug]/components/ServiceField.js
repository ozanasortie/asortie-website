...
function ServiceField({ t, data }) {
  ...
  return (
    <div className="max-w-[1500px] 2xl:max-w-[1700px] w-[90%] p-4 bg-cover bg-center bg-no-repeat relative flex flex-col items-center mt-5">
      <ArchitectService t={t} />
      <div className="text-2xl mb-2  min-w-[100%] pb-4 text-center">
        {t("cok_sorulanlar")}
      </div>
      ...
      {tabsContent.map((tab, index) => (
        ...
        >
          ...
          <AccordionPanel
            ...
            pb={0}
          >
            <div className="px-4" dangerouslySetInnerHTML={{ __html: tab.content }} />
          </AccordionPanel>
        </AccordionItem>
      ))}
    </Accordion>
